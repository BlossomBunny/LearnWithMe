...
const CHARS = {
  cyrene: {
    ...
    quizExcellent: "わあ (waa)… this score is truly wonderful. I’m so proud of how far you’ve come. 🌸✨",
...
function getChar() {
  return CHARS[ACTIVE_CHAR] || CHARS.cyrene;
}
...
function charSay(key) {
  const c = getChar();
  const v = c[key];
  if (!v) return "Let's study together! 🌸";

  if (key === 'greetings') {
    const period = timeGreeting();
    const pool = c.greetings[period] || c.greetings.morning;
...
function renderPortrait(el, charId) {
  if (!el) return;
  const c = CHARS[charId || ACTIVE_CHAR] || CHARS.cyrene;
  if (c.portrait) {
    el.innerHTML = `<img src="${c.portrait}" alt="${c.name}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
    el.style.background = 'transparent';
  } else {
    el.style.background = c.color;
    el.innerHTML = `<span style="font-size:28px">${c.emoji}</span>`;
  }
}
